...
- `npm install` / `npm ci` must never write or replace `.env`.
...
- For every Docker Compose subprocess owned by this verifier, preserve the caller environment and Harness-injected `COMPOSE_PROJECT_NAME`, but explicitly override only `API_HOST_PORT=0`.
...
- Always attempt Compose cleanup in `finally`, including on verification failure.
...
- The verifier must work with the Harness's isolated `COMPOSE_PROJECT_NAME`; it must not invent a fixed Compose project name.
...
3. With caller `API_HOST_PORT` absent, the verifier explicitly gives its Compose subprocesses `API_HOST_PORT=0` and does not depend on host port 3000 being free.
